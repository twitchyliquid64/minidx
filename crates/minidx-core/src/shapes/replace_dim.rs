...
mod tests {
    ...
    #[test]
    fn test_remove_dim_to() {
        RemoveDimTo::check(&(4, 2), &(4,));

        let tensor = (8usize, 4usize, 2usize);
        assert_eq!(tensor.remove(()), (4, 2));
    }

    #[test]
    fn test_replace_dim_to() {
        ReplaceDimTo::check(&(4, 2), &(4,));

        let tensor = (8usize, 4usize, 2usize);
        assert_eq!(tensor.replace((0,)), (0, 4, 2));
    }
}
